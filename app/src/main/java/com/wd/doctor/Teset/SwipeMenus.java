...
//左滑删除(将布局抱起来就行)
public class SwipeMenus extends ViewGroup {
    private int downX, moveX, moved;
    private Scroller scroller = new Scroller(getContext());
    private boolean haveShowRight = false;
    public static SwipeMenus SwipeMenus;


    public SwipeMenus(Context context) {
        super(context);
    }


    public SwipeMenus(Context context, AttributeSet attrs) {
        super(context, attrs);
    }

    public SwipeMenus(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        switch (ev.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                if (SwipeMenus != null && SwipeMenus == this && haveShowRight) {
                    closeMenu();
                    return true;
                }
